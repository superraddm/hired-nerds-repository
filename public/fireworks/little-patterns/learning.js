...
(function(root){
  ...
  function patternHint(round){const rule=PATTERN_RULES[round.rule]||PATTERN_RULES.repeat;if(round.rule==='number')return rule.hint.replace('{step}',{1:'ones',2:'twos',5:'fives',10:'tens'}[round.step]||'steps');if(round.rule==='repeat')return round.level<=7?round.unit.map(s=>s[0].toUpperCase()+s.slice(1)).join('. ')+'. Repeat.':'';return rule.hint;}
