...
(function(root){
  ...
  const WORD_SYMBOLS={APPLE:'apple',SUN:'sun',TREE:'tree',BALL:'ball',CAT:'cat',DOG:'dog',BUS:'bus',FISH:'fish',MOON:'moon',FLOWER:'flower',RUN:'run',SWIM:'swim',TALL:'tall',HOT:'hot',BIG:'big',JUMP:'jump',READ:'read',SING:'sing',LOUD:'loud',FAST:'fast',DRY:'dry',WET:'wet',ASLEEP:'asleep',SOFT:'soft'};
  ...
  const LEVELS={count:2,add:2,numbers:2,patterns:7};
  ...
  function levelRange(level){return level>=2?10:5;}
  function countSequence(level){return Array.from({length:levelRange(clampLevel('count',level))},(_,i)=>i+1);}
  function sumSequence(level,operation='add'){const range=levelRange(clampLevel('add',level));const sums=[];for(let total=2;total<=range;total++)for(let a=total-1;a>=1;a--)sums.push([a,total-a]);return sums;}
  function countRound(index,prefs,level=1){level=clampLevel('count',level);const sequence=countSequence(level);const target=sequence[index%sequence.length];return {id:identify('count',level,'count',target),level,target,choices:options(target,sequence,prefs.choices,index),seen:[],hint:0,done:false};}
  // pair optionally names a directly chosen sum; it must still fit the level.
  function sumRound(index,prefs,level=1,operation='add',pair=null){level=clampLevel('add',level);if(!OPERATIONS.includes(operation))operation='add';const sums=sumSequence(level,operation);const range=levelRange(level);const [a,b]=Array.isArray(pair)&&pair.length===2&&pair.every(n=>Number.isInteger(n)&&n>=1)&&pair[0]+pair[1]<=range?pair:sums[index%sums.length];return {id:identify('add',level,operation,a+'+'+b),level,operation,a,b,total:a+b,answer:a+b,stage:'parts',draft:'',choices:options(a+b,Array.from({length:range},(_,i)=>i+1),3,index),done:false};}
  ...
  function numberWordRound(index,prefs,level=1){level=clampLevel('numbers',level);const range=levelRange(level);const target=index%range+1;const word=NUMBER_WORDS[target];return {id:identify('numbers',level,'word',target),level,target,word,draft:'',choices:options(word,NUMBER_WORDS.slice(1,range+1),prefs.choices,index),done:false};}
