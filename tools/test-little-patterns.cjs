...
test('pattern rounds repeat a unit, hide one bead after the unit is shown, and every level is explicit',()=>{for(const p of combos)for(let level=1;level<=learning.LEVELS.patterns;level++)for(let i=0;i<12;i++){const r=learning.patternRound(i,p,level);assert.equal(r.level,level,'the level never derives from the round index');assert.ok(r.sequence.length===6||r.sequence.length===8);r.sequence.forEach((s,k)=>assert.equal(s,r.unit[k%r.unit.length]));assert.ok(r.gap>=r.unit.length&&r.gap<r.sequence.length);assert.equal(r.answer,r.sequence[r.gap]);assert.ok(r.choices.includes(r.answer));assert.equal(new Set(r.choices).size,p.choices);}const p=combos[0];assert.deepEqual([1,2,3,4,5,6,7].map(level=>learning.patternRound(0,p,level).unit.length),[2,2,3,3,4,4,4]);assert.equal(learning.patternRound(0,p,1).gap,5);assert.ok(learning.patternRound(3,p,2).gap<5);assert.equal(learning.patternRound(0,p,5).sequence.length,8);assert.equal(new Set(learning.patternRound(0,p,7).unit).size,4);assert.equal(learning.PATTERN_LEVELS,7);assert.equal(learning.patternRound(30,p).level,1,'an omitted level is level 1, not a promotion');assert.equal(learning.patternRound(0,p,99).level,1);assert.notDeepEqual(learning.patternRound(0,p,1).sequence,learning.patternRound(1,p,1).sequence,'Next varies the example within the level');});
