...
test('preferences are validated and unknown values fall back to the quiet defaults',()=>{const p=learning.normalisePrefs({range:20,choices:9,case:'MIXED',addition:'quiz',numberWords:'x',keyboard:'y',pace:100,soft:'yes',customWords:['dinosaur','<b>x</b>','ice  cream','dinosaur',42]});assert.deepEqual(p,{...learning.defaults,customWords:['DINOSAUR','BXB','ICE CREAM']});assert.deepEqual(learning.normalisePrefs({customSentences:['Arthur likes TRAINS.','no gap here','TWO CAPS here','x','We go to the PARK!',42,'Arthur likes TRAINS.']}).customSentences,['Arthur likes TRAINS.','We go to the PARK.']);assert.equal(learning.normalisePrefs(null).range,5);assert.equal(learning.normalisePrefs({pace:1800,soft:true}).pace,1800);assert.equal(learning.defaults.addition,'choose');assert.equal(learning.normalisePrefs({addition:'type'}).addition,'type');assert.equal(learning.normalisePrefs({addition:'quiz'}).addition,'choose');assert.equal(learning.normalisePrefs({addition:'show'}).addition,'choose','the old stored default must not skip the answer step');assert.equal(learning.normalisePrefs({addition:'demo'}).addition,'demo');});
...
test('the second sentence bank uses the same symbol words with safe distractors, and custom sentences get gaps, distractors and no near misses',()=>{
  const bath=learning.SENTENCES.filter(s=>s.scene==='bath');assert.ok(bath.length>=12);
  assert.ok(bath.some(s=>learning.sentenceWords(s.text).length>=5),'longer sentences for Word order');
  for(const s of learning.SENTENCES){assert.ok(learning.WORD_SYMBOLS[s.gap],'gap word has a symbol: '+s.gap);for(const w of learning.sentencePool(s))assert.ok(learning.WORD_SYMBOLS[w]);assert.ok(learning.sentencePool(s).length>=3);}
  for(const [word,excluded] of [['WET',['DRY','SOFT']],['JUMP',['RUN','SWIM']],['ASLEEP',['SOFT','WET']]]){const entry=learning.SENTENCES.find(s=>s.gap===word&&s.scene==='bath');const pool=learning.sentencePool(entry);for(const x of excluded)assert.ok(!pool.includes(x),word+' must not offer '+x);}
  const prefs=learning.normalisePrefs({customSentences:['Arthur likes TRAINS.','We go to the PARK.','The CAT is here.']});
  const entries=learning.sentenceEntries(prefs);assert.equal(entries.length,learning.SENTENCES.length+3);
  const custom=entries.slice(-3);assert.deepEqual(custom.map(e=>e.gap),['TRAINS','PARK','CAT']);assert.equal(custom[0].picture,null);assert.equal(custom[2].picture,'cat','a custom gap that is a symbol word keeps its picture');
  for(let i=learning.SENTENCES.length;i<entries.length;i++){const r=learning.sentenceRound(i,prefs);assert.equal(r.custom,true);assert.ok(r.gapAt>=0&&r.words[r.gapAt]===r.gap);assert.equal(r.choices.length,3);assert.equal(new Set(r.choices).size,3);assert.ok(r.choices.includes(r.gap));assert.match(r.id,/^sentence:v2:L1:custom:/);const o=learning.orderRound(i,prefs);assert.equal(o.custom,true);assert.notEqual(o.tiles.join(' '),o.words.join(' '));assert.deepEqual(o.tiles.slice().sort(),o.words.slice().sort());r.done=true;assert.deepEqual(learning.successLine('sentence',r).speech.filter(x=>!learning.OPENERS.includes(x)),[],'a private sentence is never sent to a voice automatically');}
