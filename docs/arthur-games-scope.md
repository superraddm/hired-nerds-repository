...
| Missing word | The gap in the sentence is the answer box: read (or tap the sentence to hear it while sound is on), type the word into the gap on the A to Z keys or the device keyboard, tap Enter. Three plain word cards say their word while sound is on. Give me a clue first shows the cards' Mulberry pictures, then outlines the word that fits, never filling it. The keys end in Delete, Clear and Enter; the sentence picture stays visible; one prompt in Nook's bubble. Two banks (picnic; bath and bed time) using the same 24 symbol words, plus up to twelve grown-up sentences with the gap in capitals (validated with a reason in the Grown-ups form), distractors from the other familiar gaps, no picture for words without a symbol and the local device voice. The four Word activities are chosen in the puzzle picker, not in a tab row. |
...
| Number words | Four levels: ONE to FIVE, to TEN, ELEVEN to TWENTY, and TEN to ONE HUNDRED by tens, drawn on the same tray as Count; show, choose or type the word (keys end in Delete, Clear, Space and Enter). |
...
Current result (16 September 2026, after the spelling simplification): 98 passing checks covering
